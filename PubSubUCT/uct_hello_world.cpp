...
{
    am_bcopy_args_t *bc_args = static_cast<am_bcopy_args_t *>(arg);
                                ...
                                unsigned flags)
{
    ...
    print_strings("callback", "local", "remote", length);
    ...
    rdesc = static_cast<recv_desc_t *>(malloc(sizeof(*rdesc) + length));
                                  ...
                                  iface_info_t *iface_p)
{
    ...
    for (cmpt_index = 0; cmpt_index < num_components; ++cmpt_index) {
        ...
        component_attr.md_resources = static_cast<uct_md_resource_desc_t*>(alloca(sizeof(*component_attr.md_resources) *component_attr.md_resource_count));
        status = uct_component_query(components[cmpt_index], &component_attr);
...
int main(int argc, char **argv)
{
    ...
    if (cmd_args.server_name) {
        ...
        rdesc = static_cast<recv_desc_t *>(desc_holder);
